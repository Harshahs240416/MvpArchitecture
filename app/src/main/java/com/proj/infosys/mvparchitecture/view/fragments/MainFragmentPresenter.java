...
public class MainFragmentPresenter{
    ...
    public MainFragmentPresenter (MainFragmentView mainFragmentView, WebServiceImpl webServiceObj, View view){
    ...
    public void init() {
        ...
        if (!isNetworkAvailable()) {
            if (retrieveDataFromSharedPrefs(mView.getContext().getString(R.string.shared_Pref)) != null) {
                mMainFragmentView.dismissProgressDialog();
                mMainFragmentView.setAdapter(mActionBarTitle, mRowLists);
            }else{
    ...
    public void getResponseData(){
        ...
        webService.getDataFromWebService("https://dl.dropboxusercontent.com/s/2iodh4vg0eortkl/", new WebServiceInterface() {
            ...
            public void getDataFromWebService(List<Row> rowData, String headerTitle) {
                ...
                mRowLists = rowData;
                saveDataToSharedPrefs(mView.getContext().getString(R.string.shared_Pref), rowData);
                mMainFragmentView.dismissProgressDialog();
                mMainFragmentView.setAdapter(mActionBarTitle, mRowLists);
            }
        });
    ...
    private boolean isNetworkAvailable() {
        ConnectivityManager connectivityManager
                = (ConnectivityManager) mView.getContext().getSystemService(mView.getContext().getApplicationContext().CONNECTIVITY_SERVICE);
        NetworkInfo activeNetworkInfo = connectivityManager.getActiveNetworkInfo();
        return activeNetworkInfo != null && activeNetworkInfo.isConnected();
    }

    public void saveDataToSharedPrefs(String sharedPrefName, List<Row> responseData) {
        SharedPreferences sp = mView.getContext().getSharedPreferences(sharedPrefName, Context.MODE_PRIVATE);
        Gson gson = new Gson();
